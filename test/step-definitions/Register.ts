...
Then(/^if user provides invalid email id then error is populated on screen$/, async function () {
    await Loginpage.createEmailAndNavigate("Divya1");
    expect(await Loginpage.errorMessage()).toBe("Invalid email address.")
});
